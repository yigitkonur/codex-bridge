...
```json
{
  ...
  "command": "node \"${CLAUDE_PLUGIN_ROOT}/skill/scripts/codex-bridge.mjs\" events <job-id-or-thread-id> --follow --exclude HEARTBEAT --timeout-ms 1800000",
  "timeout_ms": 3600000,
  "persistent": false
}
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/skill/scripts/codex-bridge.mjs" events "$ARGUMENTS"
```
