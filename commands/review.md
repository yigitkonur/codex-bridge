...
```bash
node "${CLAUDE_PLUGIN_ROOT}/skill/scripts/codex-bridge.mjs" review "$ARGUMENTS"
```
...
```typescript
Bash({
  command: `node "${CLAUDE_PLUGIN_ROOT}/skill/scripts/codex-bridge.mjs" review "$ARGUMENTS"`,
