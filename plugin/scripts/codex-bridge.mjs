...
var _DOCTOR_SCRIPT_DIR = path14.dirname(fileURLToPath2(import.meta.url));
var _DOCTOR_SKILL_DIR = fs16.existsSync(path14.join(_DOCTOR_SCRIPT_DIR, "..", "schemas")) ? path14.join(_DOCTOR_SCRIPT_DIR, "..") : path14.join(_DOCTOR_SCRIPT_DIR, "..", "..");
...
function defaultWorktreeRoot2(repoRoot) {
  return path14.resolve(repoRoot, "..", WORKTREE_ROOT_NAME);
}
...
function checkOrphanWorktrees(repoRoot, workspaceRoot, options = {}) {
  ...
  for (const entry of safeReadDir(worktreeRoot)) {
    ...
    const worktreePath = path14.join(worktreeRoot, entry.name);
    let stat = null;
    try {
      stat = fs16.statSync(worktreePath);
    } catch {
    }
    findings.push({
...
function checkOldSessionFiles(cwd, workspaceRoot, options = {}) {
  ...
  for (const entry of safeReadDir(sessionDir)) {
    ...
    const filePath = path14.join(sessionDir, entry.name);
    let stat;
    try {
...
function checkDiskUsage(cwd, workspaceRoot) {
  ...
  const checks = [
    ...
    { type: "disk_usage", severity: "info", label: "codex_rollouts", ...directorySizeBytes(path14.join(os7.homedir(), ".codex", "sessions")) }
  ];
...
function applyDoctorAction(finding, context, options = {}) {
  ...
  if (finding.action === "remove_worktree") {
    ...
    if (remove.status !== 0 && finding.path.includes(`${path14.sep}${WORKTREE_ROOT_NAME}${path14.sep}`)) {
      fs16.rmSync(finding.path, { recursive: true, force: true });
    }
    return {
      finding,
      action: finding.action,
      cleaned: !fs16.existsSync(finding.path),
      skipped: false,
...
var SCRIPT_DIR = path15.dirname(fileURLToPath3(import.meta.url));
