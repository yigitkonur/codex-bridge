...
| `pipeline:<stage>` | `<stage>` matches `failing_stage:`; `PIPELINE_ERROR.lastCompletedStage` records prior progress. Rerun review from the task worktree with `--cwd <worktree.path> --base <worktree.base_ref>`. |
