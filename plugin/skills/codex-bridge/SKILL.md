...
- **Stop** uses a fast path by default: it scans pending verdicts and returns immediately, blocking only to list unresolved verdicts. Set `stop_review_gate.enabled: true` and `stop_review_gate.fast_scan_only: false` in config when you explicitly want the slower stop-time Codex review.
