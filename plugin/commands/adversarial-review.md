...
argument-hint: "[--wait|--background] [--backend <name>] [--task <task_id>] [--base <ref>] [--scope auto|working-tree|branch] [--json] [focus ...]"
...
- With `--json`, stdout is a bridge envelope whose `result.result` is the raw adversarial schema output and whose `result.review_result` is the normalized contract.
- `result.review_result` contains `schema_version`, `review_kind`, `verdict`, `summary`, `findings`, `next_steps`, `target`, `task_id`, `reviewed_branch_head_sha`, and `raw_output`.
