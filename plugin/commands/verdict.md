...
=======
- **Write from stdin** (`--payload-stdin`): reads a JSON object with `verdict`, optional `summary`, `findings`, and `reviewer`. Use this when values come from review output so findings are not interpolated into shell arguments.
