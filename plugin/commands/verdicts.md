...
Pass `--json` to receive the structured `{ count, pending: [{ task_id, verdict, summary, decided_at, branch }] }` payload (the form the Stop hook consumes). The bare invocation is human-readable: `task_id  verdict  branch  summary` per line.
