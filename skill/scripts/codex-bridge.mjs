...
// src/adapters/codex/index.mjs
var ADAPTER_DIR = path11.dirname(fileURLToPath3(import.meta.url));
var SOURCE_ROOT_DIR = path11.resolve(ADAPTER_DIR, "../..");
var BUNDLE_ROOT_DIR = path11.resolve(ADAPTER_DIR, "..");
var CONFIG_ROOT_DIR = fs13.existsSync(path11.join(SOURCE_ROOT_DIR, "schemas")) ? SOURCE_ROOT_DIR : fs13.existsSync(path11.join(BUNDLE_ROOT_DIR, "schemas")) ? BUNDLE_ROOT_DIR : null;
...
var LOCAL_CONFIG_RELATIVE_PATH = path14.join(".claude", "codex-bridge.local.md");
var DEFAULT_BODY = [
  "",
...
function loadExecuteInstructions(rootDir) {
  const p = path15.join(rootDir, "templates", "execute-instructions.md");
  try {
...
function listLocalBranches(repoRoot) {
  const result = spawnSync5("git", ["branch", "--list", `${BRANCH_PREFIX}*`], {
    cwd: repoRoot,
    encoding: "utf8",
    timeout: 1e4
  });
...
function listGitWorktrees(repoRoot) {
  const result = spawnSync5("git", ["worktree", "list", "--porcelain"], {
    cwd: repoRoot,
    encoding: "utf8",
    timeout: 1e4
  });
...
async function runDoctorChecks(cwd) {
  ...
  const gitRoot = spawnSync5("git", ["rev-parse", "--show-toplevel"], {
    cwd: workspaceRoot,
    encoding: "utf8",
    timeout: 5e3
  });
...
function worktreeStatus(worktreePath) {
  const result = spawnSync5("git", ["-C", worktreePath, "status", "--porcelain", "--untracked-files=all"], {
    encoding: "utf8",
    timeout: 1e4
  });
...
function applyDoctorAction(finding, context, options = {}) {
  ...
  if (finding.action === "remove_worktree") {
    ...
    const remove = spawnSync5("git", ["worktree", "remove", "--force", finding.path], {
  ...
  if (finding.action === "delete_branch") {
    ...
    const result = spawnSync5("git", ["branch", "-D", finding.branch], {
      cwd: context.repoRoot,
      encoding: "utf8",
      timeout: 3e4
    });
...
var COMMANDS = Object.freeze({
  ...
  status: {
    synopsis: "status [job-id] [--all] [--filter running|completed_success|completed_fail|completed_incomplete|cancelled|needs_attention] [--wait] [--watch [--interval 10s] [--watch-timeout-ms <ms>]] [--prune-orphans|--cleanup [--dry-run] [--retention-days <n>] [--retention-jobs <n>]] [--timeout-ms <ms>] [--poll-interval-ms <ms>] [--json]",
    summary: "List jobs, or inspect one by id. Multi-job status includes event-derived summary counts and needs_attention for failed/incomplete/interrupted jobs; running === 0 is only the terminal-state barrier. With --wait, poll one job to terminal. With --watch, repeatedly render the multi-job table and exit when all tracked jobs reach terminal state (Ctrl-C-safe). Use --watch for N-job orchestration.",
    examples: [
      "codex-bridge status",
...
function readTaskPrompt(cwd, options, positionals) {
  if (options["prompt-file"]) {
    return readPromptFileOrThrow2(path20.resolve(cwd, options["prompt-file"]));
  }
...
function resolvePromptInput(options, positionals, cwd) {
  if (options["prompt-file"]) {
    return readPromptFileOrThrow2(path21.resolve(cwd, options["prompt-file"]));
  }
...
async function handleTaskWorker(argv) {
  ...
  const { logFile, progress } = createTrackedProgress2(
    {
      ...storedJob,
      workspaceRoot
    },
...
async function handleStatus(argv) {
  const startedAt = Date.now();
  const { options, positionals } = parseCommandInput(argv, {
    valueOptions: ["cwd", "group", "timeout-ms", "poll-interval-ms", "interval", "watch-timeout-ms", "retention-days", "retention-jobs", "filter"],
...
async function runIterateTaskJob({
  ...
  const { logFile } = createTrackedProgress2(job, { stderr: false });
  const execution = await runTrackedJob(
    job,
