...
>>>>>>> cb6659c (test: stabilize baseline tests on PR #55)
...
function createSubagentWorktree({
  ...
  try {
    ...
    if (partialBranchSha?.trim() === baseSha) {
...
function loadConfig(skillDir, overrideDir = null, workspaceRoot = null) {
  ...
  const workspaceLayer = workspaceConfigPath && fs10.existsSync(workspaceConfigPath) ? readYaml(workspaceConfigPath) : {};
  const overrideConfigPath = overrideDir ? path8.join(overrideDir, "config.yaml") : null;
  const overrideLayer = overrideConfigPath && fs10.existsSync(overrideConfigPath) ? readYaml(overrideConfigPath) : {};
  return {
    ...DEFAULT_CONFIG,
    ...skillLayer,
    ...workspaceLayer,
...
function buildUntrackedFileStat(cwd, fileName) {
  ...
  let adds = 0;
  try {
    const stat = fs10.lstatSync(absolutePath);
    sizeBytes = stat.size;
    if (stat.isFile() && stat.size <= MAX_UNTRACKED_STAT_BYTES) {
      const content = fs10.readFileSync(absolutePath);
...
function resolveInsideCwd(cwd, fileName) {
  ...
  const absolutePath = path8.resolve(root, fileName);
  if (absolutePath !== root && !absolutePath.startsWith(root + path8.sep)) {
    return null;
  }
  return absolutePath;
}
...
function readCapturedDiffContent(diff) {
  ...
  try {
    return fs12.readFileSync(diff.diffPath, "utf8");
  } catch {
    return "";
  }
}
...
function acquireCacheLock(staleMs) {
  ...
  try {
    fs13.mkdirSync(path11.dirname(lockPath), { recursive: true });
  } catch {
    return null;
  }
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      try {
        fs13.writeFileSync(fd, JSON.stringify({ pid: process.pid, createdAt: Date.now() }));
      } catch {
      }
      return { fd, lockPath };
    } catch (err) {
...
`;
      ...
      child.on("error", () => {
        try {
          fs14.appendFileSync(logFile, `[${(/* @__PURE__ */ new Date()).toISOString()}] spawn failed (npx not on PATH?)
`, "utf8");
        } catch {
        }
      });
...
`;
    ...
    child.on("error", () => {
      try {
        fs15.writeSync(fd, `[${(/* @__PURE__ */ new Date()).toISOString()}] spawn failed (npx not on PATH?)
`);
      } catch {
      }
    });
...
=======
  return options.cwd ? path13.resolve(process8.cwd(), options.cwd) : process8.cwd();
>>>>>>> cb6659c (test: stabilize baseline tests on PR #55)
}
...
async function handleTaskWorker(argv) {
  ...
  const cwd = resolveCommandCwd(options);
  const workspaceRoot = options["workspace-root"] ? path12.resolve(process9.cwd(), options["workspace-root"]) : resolveCommandWorkspace(options);
...
=======
    ...
    const ts = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
    const file = path13.join(crashDir, `${ts}-${process8.pid}.log`);
>>>>>>> cb6659c (test: stabilize baseline tests on PR #55)
    const payload = {
      kind,
      ts,
