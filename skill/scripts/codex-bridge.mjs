...
function buildTaskRequest({ cwd: cwd2, model, effort, prompt, write, resumeLast, jobId, mode, idleTimeoutMs }) {
  return {
    cwd: cwd2,
    model,
    effort,
    ...
    jobId,
    mode: mode ?? null,
    idleTimeoutMs: Number.isFinite(Number(idleTimeoutMs)) && Number(idleTimeoutMs) > 0 ? Number(idleTimeoutMs) : null
  };
}
