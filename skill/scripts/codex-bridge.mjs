...
var DEFAULT_TASK_WAIT_TIMEOUT_MS = 18e5;
var TASK_WAIT_EVENT_REGEX = /^\[(DONE|ERROR|INCOMPLETE|CANCELLED|PLAN|PLAN_READY|QUESTION)\]/m;
  ...
  if (tag === "PLAN" || tag === "PLAN_READY") {
    return {
      ...
      next_action: storedJob?.result?.next_action ?? {
        send: storedJob?.threadId ? `${bridgeCommand("send", cwd)} ${storedJob.threadId} --mode default "Implement the plan."` : null
      }
    };
  }
  return null;
...
function buildTaskWaitResult({ job, storedJob, terminal, startedAt, eventsPath, cwd }) {
  ...
  const interrupt = rawTag === "QUESTION" || rawTag === "PLAN" || rawTag === "PLAN_READY" ? buildTaskWaitInterrupt({ tag: rawTag, line: terminal.line, storedJob, artifacts, cwd }) : null;
  const lastAssistantMessage = typeof storedJob?.result?.rawOutput === "string" ? storedJob.result.rawOutput : typeof storedJob?.result?.lastAssistantMessage === "string" ? storedJob.result.lastAssistantMessage : null;
  return {
    jobId: job.id,
    threadId,
    terminalTag,
...
async function waitForBackgroundTaskCompletion({ cwd, workspaceRoot, job, timeoutMs, startedAt }) {
  ...
  if (threadWait.timedOut || !threadWait.job?.threadId) {
    throw new CliError(
      `Task ${job.id} did not report a thread id within ${Math.round(timeoutMs / 1e3)}s.`,
      {
        class: "timeout",
  ...
  if (terminal.timedOut) {
    throw new CliError(
      `No terminal or interrupt event for task ${job.id} within ${Math.round(timeoutMs / 1e3)}s.`,
      {
        class: "timeout",
        code: "TASK_WAIT_TIMEOUT",
        retryable: true,
