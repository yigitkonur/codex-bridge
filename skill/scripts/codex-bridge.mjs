...
var NOT_IMPLEMENTED, adapter, codex_default;
...
=======
  if (err?.name === "AdapterError" && err?.code === "BACKEND_INCAPABLE") {
...
function loadConfig(skillDir, overrideDir = null, workspaceRoot = null) {
  ...
  return {
=======
function resolveConfigPaths(skillDir, overrideDir = null, workspaceRoot = null) {
...
function loadConfigLayers(skillDir, overrideDir = null, workspaceRoot = null) {
  const { skillConfigPath, workspaceConfigPath, overrideConfigPath } = resolveConfigPaths(skillDir, overrideDir, workspaceRoot);
  const skillLayer = readBridgeYaml(skillConfigPath);
  const workspaceLayer = workspaceConfigPath && fs.existsSync(workspaceConfigPath) ? readBridgeYaml(workspaceConfigPath) : {};
  const overrideLayer = overrideConfigPath && fs.existsSync(overrideConfigPath) ? readBridgeYaml(overrideConfigPath) : {};
  const mergedConfig = {
>>>>>>> 1e991d7 (review(stage 4): apply adversarial-review findings)
    ...DEFAULT_CONFIG,
    ...skillLayer,
...
// import("./**/*/index.mjs") in src/adapters/index.mjs
var globImport_index_mjs = __glob({
  "./codex/index.mjs": () => Promise.resolve().then(() => (init_codex(), codex_exports))
});
...
async function loadAdapter(name) {
  ...
  let mod;
  try {
    mod = await globImport_index_mjs(`./${name}/index.mjs`);
  } catch (err) {
    throw new AdapterError(
      "BACKEND_INCAPABLE",
...
function formatUntrackedFile(cwd, relativePath) {
  const absolutePath = path4.join(cwd, relativePath);
  let stat;
  try {
...
(skipped: ${stat.size} bytes exceeds ${MAX_UNTRACKED_BYTES} byte limit)`;
  ...
  let buffer;
  try {
    buffer = fs4.readFileSync(absolutePath);
  } catch {
    return `### ${relativePath}
(skipped: broken symlink or unreadable file)`;
  }
...
var CodexAppServerClient = class {
  static async connect(cwd, options = {}) {
    let brokerEndpoint = null;
    if (!options.disableBroker) {
      brokerEndpoint = options.brokerEndpoint ?? options.env?.[BROKER_ENDPOINT_ENV] ?? process7.env[BROKER_ENDPOINT_ENV] ?? null;
      if (!brokerEndpoint && options.reuseExistingBroker) {
...
function resolveStopReviewGateLockPath(workspaceRoot) {
  return path11.join(workspaceRoot, STOP_REVIEW_GATE_LOCK_FILE);
}
...
function readTaskPrompt(cwd, options, positionals) {
  if (options["prompt-file"]) {
    return readPromptFileOrThrow(path11.resolve(cwd, options["prompt-file"]));
  }
...
async function handleAwaitArtifact(argv) {
  ...
  const resolvedPath = path11.isAbsolute(artifactPath) ? artifactPath : path11.resolve(cwd, artifactPath);
  const deadline = Date.now() + timeoutMs;
  let prevSize = null;
  while (true) {
    let jobSnapshot;
    ...
    let statInfo = null;
    try {
      statInfo = fs13.statSync(resolvedPath);
    } catch (e) {
...
=======
      if (passes(line)) process9.stdout.write(line + "\n");
>>>>>>> 1e991d7 (review(stage 4): apply adversarial-review findings)
...
function resolvePromptInput(options, positionals, cwd) {
  ...
  if (positionals.length === 1) {
    const candidate = path11.resolve(cwd, positionals[0]);
    try {
