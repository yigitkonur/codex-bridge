...
- **Forensic bundle:** `bundle <task-id>` writes `./codex-bridge-bundle-<task-id>.tar.gz` with `manifest.json`, `timeline.txt`, session logs, registry detail, job logs, worker stderr, and the Codex rollout when found. Use `--output <path>` to choose the tarball path or `--no-include-rollout` when conversation privacy/size matters.
