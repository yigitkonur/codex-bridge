...
test("status --json exposes failed summary and script filters", () => {
  withIsolatedState(({ workspace, sessionDir }) => {
    seedPipelineFailureFixture(workspace, sessionDir);
    ...
    const attentionResult = spawnSync(
      process.execPath,
      [BRIDGE.pathname, "status", "--cwd", workspace, "--all", "--filter", "needs_attention", "--json"],
      {
        cwd: workspace,
        encoding: "utf8",
        env: {
    ...
    assert.equal(attentionResult.status, 0, attentionResult.stderr || attentionResult.stdout);
    const attentionEnvelope = JSON.parse(attentionResult.stdout);
    assert.equal(attentionEnvelope.result.filter, "needs_attention");
    assert.deepEqual(attentionEnvelope.result.filtered_jobs.map((entry) => entry.jobId), ["task-pipeline-failed"]);
  });
});
