...
test("codex adapter result preserves cancelled status when events are missing", async (t) => {
  const previousPluginData = process.env.CODEX_BRIDGE_PLUGIN_DATA;
  const root = makeTempDir("codex-adapter-result-cancelled-missing-events-");
  const workspace = path.join(root, "workspace");
  const stateRoot = path.join(root, "state");
  fs.mkdirSync(workspace);
  process.env.CODEX_BRIDGE_PLUGIN_DATA = stateRoot;
  ...
  const job = {
    id: "task-cancelled-missing-events",
    status: "cancelled",
    phase: "cancelled",
    title: "Codex Task",
    jobClass: "task",
    workspaceRoot: workspace,
    threadId: "thread-cancelled-missing-events",
    summary: "cancelled",
    result: {},
  };
  writeJobFile(workspace, job.id, job);
