...
// Detect a sandbox.enforce / sandbox_enforce: true pin in the workspace
// config so Layer 2 can deny read-only bridge tasks regardless of how
// the orchestrator invoked the CLI. The deny rule installer (Layer 1) is
// the primary backstop; this function is the in-process safety net for
// invocations that slip past or override the user-settings deny rule.
function configTextEnforcesSandbox(text) {
  if (!text) return null;
  const flat = text.match(
    /^\s*(?:sandbox_enforce|enforce_sandbox)\s*:\s*(true|false)\s*(?:#.*)?$/m,
  );
  if (flat) return flat[1] === "true";
  const sandboxBlock = text.match(/^\s*sandbox\s*:\s*(?:#.*)?\n((?:\s{2,}[^\n]*\n?)*)/m);
  const nested = sandboxBlock?.[1]?.match(
    /^\s+enforce\s*:\s*(true|false)\s*(?:#.*)?$/m,
  );
...
function classifyCommand(command, cwd) {
  ...
  // Layer 2 of the three-layer sandbox enforcement recipe: deny --read-only
  // bridge tasks when the workspace pins sandbox.enforce: true. Layer 1 is
  // the user-settings deny rule installed by `setup --enforce-sandbox`;
  // Layer 3 is the Explore-agent default in pre-tool-agent.mjs.
...
function main() {
  ...
  const classification = classifyCommand(command, input.cwd);
