...
- [x] **PLUG-01**: Packaged plugin manifest, commands, agents, hooks, config, scripts, prompts, schemas, and templates all resolve inside the packaged `plugin/` tree. Validated in Phase 4 by recursive packaged reference checks, manifest path normalization tests, generated-surface inventory, and `npm run verify:static`.
- [x] **PLUG-02**: Root plugin metadata, package version, generated plugin metadata, and skill metadata have an explicit, tested version/canonicality relationship. Validated in Phase 4 by metadata tests and baseline metadata verification.
- [x] **PLUG-03**: Stop hook blocks only when the project lock and setup state prove the review gate is active, and it always leaves enough timeout margin to emit a decision. Validated in Phase 4 by active Stop hook lock/setup/pending-verdict/timeout tests.
- [x] **PLUG-04**: Session, subagent, user-prompt, and post-tool hooks cannot be spoofed by arbitrary stdout or unsafe monitor command text. Validated in Phase 4 by registered hook surface tests, monitor sanitization, and active spoofed-stdout/unsafe-command tests.

### State And Artifacts

- [x] **STAT-01**: Workspace state is keyed by canonical workspace root and survives multiple processes without dropping concurrent jobs. Validated in Phase 5 by state lock/concurrency tests, canonical session-dir resolution, and workspace-root call-site proof.
- [x] **STAT-02**: Session `.events` and `.ndjson` logs are append-only, readable by monitor commands, and sufficient for replaying progress and failures. Validated in Phase 5 by replay helpers and corrupt-line preservation tests.
- [x] **STAT-03**: Per-task registry artifacts persist brief, metadata, review, verdict, diff, and events in stable JSON/patch formats. Validated in Phase 5 by brief, diff, registry event, review, verdict, and metadata tests plus task/iterate persistence wiring.
- [x] **STAT-04**: Cancel, orphan pruning, stale locks, corrupt state, and missing artifacts produce structured, retry-aware outcomes. Validated in Phase 5 by recovery payload tests, stale-lock tests, corrupt-state quarantine tests, and invalid JSON response guarding.

### Release Readiness

- [x] **REL-01**: CI rejects stale generated bundles and missing packaged outputs. Validated in Phase 6 by build workflow static-gate wiring, generated drift checks, packaged output checks, and workflow tests.
- [x] **REL-02**: Release packaging builds from source, stages installable skill payloads, removes maintainer-only files from archives, and attaches checksummed artifacts. Validated in Phase 6 by `scripts/package-release.mjs`, release workflow wiring, and archive/checksum tests.
- [x] **REL-03**: Authenticated runtime smoke checks cover setup, task, review, and event streaming against a real Codex install before a release is called complete. Validated in Phase 6 by `scripts/runtime-smoke.mjs` and a live `--require-codex` run against `codex-cli 0.125.0`.
- [x] **REL-04**: Update checks and auto-apply paths are rate-limited, non-blocking on hot paths, and produce actionable diagnostics when network or installer calls fail. Validated in Phase 6 by existing update diagnostics plus targeted update and auto-apply tests.
