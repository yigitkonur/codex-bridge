...
|---|---:|---|
| `14.13 — Runner agent has no Bash permission for its only allowed tool, and doesn't fail-fast on denial` | P1 | Real, but partially mitigated in the current tree. The runner prompt and `SubagentStop` hook reduce the 150s false-success ambiguity; the missing preapproval for the runner's only Bash command remained real. |
...
|---|---|---|
| Tool availability was mistaken for command approval. | Runner frontmatter uses `tools: Bash`; Claude Code subagent frontmatter supports tool selection, not command-scoped approval. | Do not add unsupported agent frontmatter. Use permissions or hooks. |
...
| `14.09 absolute paths break worktree isolation` | Shares `PreToolUse(Bash)` safety surface; the auto-allow must run only after write/worktree safety gates. |
...
| 3 — Preserve safety denials | Wave 2 | Ensure write-without-worktree, non-bundled paths, compound shell commands, and subshell syntax do not auto-allow | Negative tests |
...
| Bypassing worktree safety | Auto-allow is emitted only on the safe/pass-through path after write/worktree checks; missing worktree isolation still denies. | Revert hook allow branch; safety denials remain. |
...
| Safety non-regression | Write tasks without `--worktree-auto`, non-bundled paths, and compound shell commands do not auto-allow. |
...
|---|---|
| `npm run build` | Pass |
| `node --test test/pre-tool-bash-hook.test.mjs test/plugin-surfaces.test.mjs` | Pass: 57 passed, 1 skipped |
| `mise x node@22.22.2 -- npm run build` | Pass |
| `TMPDIR=<fresh> mise x node@22.22.2 -- node --test --test-concurrency=1 test/*.test.mjs` | Pass: 381 passed, 1 skipped |
| `mise x node@22.22.2 -- npm run baseline:contracts -- --check` | Pass |
