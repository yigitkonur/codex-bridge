...
| Monitor default contract | Default Monitor command becomes `events <id> --follow --exclude HEARTBEAT,DIRECTIVES,CHECKPOINT --timeout-ms 1800000` | Default stream includes `[CHECKPOINT_SUMMARY]` and excludes startup/runtime echoes plus verbose `[CHECKPOINT]` | `DEFAULT_MONITOR_EXCLUDE` assertion; plugin surface expected hint |
...
| Case | Check |
|---|---|
| 14.24 | A fixture containing `[DIRECTIVES]`, `[CHECKPOINT_SUMMARY]`, verbose `[CHECKPOINT]`, and terminal tags streamed with `--exclude HEARTBEAT,DIRECTIVES,CHECKPOINT` shows summary/terminal tags and hides directives plus the verbose checkpoint body. |
