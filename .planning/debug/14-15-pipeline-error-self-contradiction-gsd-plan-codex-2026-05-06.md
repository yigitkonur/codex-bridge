...
|---|---|---|
| `codex-bridge-feedback/codex/14-real-world-failure-cases/15-P1-pipeline-error-self-contradiction.md` | `PIPELINE_ERROR` has internally contradictory fields | Real P1 defect, with one overstated subclaim: `reviewVerdict: "approved"` on failed review is confirmed; `origin: pipeline:diff` is confusing but currently documented as last-completed-stage compatibility, not necessarily the failed stage. |
...
| `origin: "pipeline:diff"` contradicts `failing_stage: "review"`. | Overstated in current repo, but still a contract hazard. | Current references document `origin: pipeline:<lastCompleted>` and tell readers to use `failing_stage` for the actual stalled stage. However some docs still say `pipeline:<stage>` loosely, and the name `origin` invites consumers to treat it as the failed stage. |
...
| B. Origin/failing-stage semantics | `origin` has compatibility semantics but its name and docs are easy to misread as failed-stage source. | `skill/references/notification-format.md`, `skill/references/error-recovery.md`, `skill/references/ndjson-guide.md`, plugin skill references as needed, `test/session-log.test.mjs` | `failing_stage` is canonical failed stage; `origin` is compatibility routing/last-completed context unless a deliberate breaking migration changes it. |

Recommended option: fix A fully and clarify B without changing `origin` semantics in this P1. Changing `origin` from `pipeline:<lastCompleted>` to `pipeline:<failing_stage>` is a breaking contract for existing monitor filters and recovery tables; if product wants that, do it as a separate schema migration with compatibility fields.
...
| 2. Clarify stage semantics | Add an explicit `lastCompletedStage` field to `PIPELINE_ERROR` and `result.pipeline` or document `origin` as the compatibility alias for it. Keep `failing_stage` as the only canonical failed-stage field. | Wave 1, because docs should describe implemented behavior. | Event tests assert `failing_stage: "review"`, `lastCompletedStage: "diff"`, and documented `origin` behavior. |
...
| File | Change |
|---|---|
| `src/adapters/codex/pipeline.mjs` | Prefer adding `lastCompletedStage` to error payloads and return values. Preserve `origin: pipeline:<lastCompletedStage>` unless a migration explicitly chooses the breaking origin change. |
| `src/lib/session-log.mjs` | Keep `[ERROR]` actions branching on pipeline origin plus `failingStage`. If `lastCompletedStage` is rendered, label it clearly as context, not failure source. |
| `skill/references/notification-format.md` | State: `failing_stage` is canonical; `origin: pipeline:<lastCompleted>` is compatibility routing/context; review fields are `null`/absent unless review completed. |
...
- Consumers can branch on `failing_stage` without guessing from `origin`.
...
- `formatErrorEvent` still renders pipeline recovery actions for `origin: pipeline:diff` and `failingStage: review`.
- Docs and tests agree on `origin` semantics.
- No plan requires consumers to parse the `error` string to know the failed stage.

Rollback:

- If `lastCompletedStage` causes schema concerns, remove that additive field and keep the documentation-only clarification. Do not flip `origin` semantics in rollback; that is a separate migration decision.
...
| Changing `origin` breaks monitors that use `origin.startsWith("pipeline:")` and recovery docs keyed by last completed stage. | Preserve `origin` in this P1; add/clarify `lastCompletedStage` and make `failing_stage` canonical. |
...
| `14.15` origin semantics | Docs and tests identify `failing_stage` as the failed stage and `origin` as compatibility routing/last-completed context, or a deliberate migration test proves `origin === "pipeline:${failing_stage}"` if the product chooses the breaking option. |
