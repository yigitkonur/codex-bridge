...
`--no-pipeline` was described as a way to skip slow review/fix/check validation. In the live source, the main Codex turn still runs before the no-pipeline branch, and Codex edits happen during that turn via file-change/tool activity. The bridge defect was not that a separate "diff application" stage failed to apply edits; the pipeline diff stage captures and reports diffs. The actual failure was that a write-mode turn with no touched files and no new git delta still received a success terminal.
...
- That diff capture was workspace-cumulative, so pre-existing dirty files could appear as task output.
...
| No-op write guard | Zero-output write turns could emit `[DONE]` | `src/lib/task-runtime.mjs`, `src/adapters/codex/pipeline.mjs` | Write-mode tasks with no touched files and no git-state delta emit `[INCOMPLETE] no_files_touched` |
| Regression/docs | Tests/docs encoded old ambiguity | `test/handler-runtime.test.mjs`, `skill/*`, generated bundles | The observable behavior is pinned and documented |
...
3. Runtime fix: pass task-start snapshot and touched files into pipeline; make no-work incomplete before `[DONE]`.
4. Docs/build: update skill references, run `npm run build`.
5. Verification: targeted runtime/pipeline/static suites; full suite triage.
...
| Preserve task-vs-workspace diff truth | Task diff and workspace diff stay separately rendered | `node --test test/auto-pipeline-turn-watchdog.test.mjs` |
...
| Pre-existing dirty files confuse task output | Snapshot comparison avoids treating unchanged dirty state as new work | Revert to direct diff capture only |
...
Implemented and verified with `npm run build`, targeted runtime/pipeline/static/baseline suites, and full `npm test` (420 passing, 1 skipped, 0 failing).

## Out of scope

- Changing default pipeline timeout budgets beyond current repo state.
- Redesigning terminal envelope semantics for all error classes.
- Fixing cumulative workspace diff reporting outside the task/no-pipeline guard.
